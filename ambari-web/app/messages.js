...
Em.I18n.translations = {
  ...
  'installer.step2.manualInstall.info':'如果不使用SSH连接到目标主机，则必须手动安装和在每个主机上启动Ambari代理，以便向导执行必要的配置软件安装。',
  ...
  'installer.step2.localRepo.tooltip.content': '群集安装需要访问网络才能获取软件。在某些情况下，没有足够的带宽可用，您希望阻止下载一次又一次地从远程存储库获取包。其他时候，群集中的主机无法访问网络。在这些情况下，您必须设置您的计算机可以访问本地，这称为<b>本地软件库</b>',
  'installer.step2.javaHome.label' : 'Path to 64-bit JDK',
  'installer.step2.javaHome.tooltip.title' : 'JAVA_HOME',
  'installer.step2.javaHome.tooltip.content' : '64位JDK的JAVA_HOME。 /usr/jdk/jdk1.6.0_31是Ambari使用的默认路径。您也可以指定其他包含JDK的路径。 <br/> 注意: 路径必须在群集中的<b>所有</b>主机上有效。',
  'installer.step2.javaHome.tooltip.placeholder' : '/usr/jdk/jdk1.6.0_31',
  'installer.step2.automaticInstall.tooltip.title':'自动注册',
  'installer.step2.automaticInstall.tooltip.content':'在安装群集之前，Ambari将自动在每个主机上安装并注册Ambari代理。',
  'installer.step2.useSsh.provide' : '提供',
  ...
  'installer.step2.manualInstall.tooltip.content':'在每台主机上手动注册Ambari Agent无需SSH，应在继续安装群集之前执行。',
  'installer.step2.manualInstall.tooltip.content_no_ssh':'在继续安装群集之前，应在每个主机上手动注册Ambari Agent。',
  'installer.step2.manualInstall.popup.header':'继续之前',
  'installer.step2.manualInstall.popup.body':'在继续之前，必须在要管理的每个主机上安装Ambari Agent。',
  'installer.step2.warning.popup.body':'<p>以下主机名不是有效的FQDN：</p><p> {0} </p><p>这可能会在安装过程中导致问题。是否要继续？</p>',
  ...
  'installer.step2.evaluateStep.continueConfirm':'您是否要继续？',
  'installer.step2.evaluateStep.hostRegInProgress':'主机注册当前正在进行。请稍后再试。',
  'installer.step2.sshUser':'SSH连接用户',
  'installer.step2.sshUser.toolTip':'用于通过SSH在目标主机上安装Ambari Agent的用户帐户。必须在所有目标主机上为该用户设置无密码SSH和sudo访问权限',
  'installer.step2.sshUser.placeholder':'输入用户名称',
  'installer.step2.sshUser.required':'需要用户名',
  'installer.step2.sshPort':'SSH Port Number',
  'installer.step2.sshPort.toolTip':'SSH连接端口',
  'installer.step2.sshPort.required':'需要SSH连接端口。',
  'installer.step2.agentUser':'Ambari Agent 用户账户',
  'installer.step2.agentUser.toolTip':'用于在目标主机上运行Ambari Agent守护程序的用户帐户。必须在所有目标主机上设置此用户的无密码sudo访问权限',
  'installer.step2.bootStrap.error':'在主机上设置Ambari Agents时遇到错误。',
  'installer.step2.bootStrap.inProgress':'正在主机上设置Ambari Agents，请稍候。这可能需要几分钟的时间，具体取决于主机的数量。',
  'installer.step2.bootStrap.header':'设置 Ambari Agents',
  'installer.step2.skipHostChecks.label': '跳过主机检查',
  'installer.step2.skipHostChecks.popup.header': '警告',
  'installer.step2.skipHostChecks.popup.body': '若跳过主机检查，如果发现主机有任何问题，Ambari将不会进行检查并发出警告，主机将按原样添加到集群中。',
  ...
  'installer.step3.hosts.remove.popup.body':'您是否确实要删除选定的主机？',
  'installer.step3.hostInformation.popup.header':'检索主机信息时出错',
  'installer.step3.hostInformation.popup.body' : '已注册所有引导主机，但无法检索与cpu和内存相关的信息',
  'installer.step3.hostOsTypeCheck.popup.body' : '主机已成功注册，但主机操作系统类型与“选择堆栈”步骤中的选定组不匹配：高级存储库选项。您可以返回“选择堆栈”步骤或删除此主机。主机类型为{0}，但您在步骤1中选择了{1}组。',
  'installer.step3.hostWarningsPopup.report':'展示报告',
  'installer.step3.hostWarningsPopup.report.header': '<p style="font-family: monospace">######################################Generated<br>#主机检查报告<br>#<br>: ',
  ...
  'installer.step3.hostWarningsPopup.jdk.name':'在<i>{0}</i>并未找到JDK',
  ...
  'installer.step3.hostWarningsPopup.repositories.name':'OS存储库不可用',
  'installer.step3.hostWarningsPopup.repositories.context':' ({0})主机是{1}OS类型, 但在“选择堆栈”步骤中选择的存储库是{2}。选定的存储库不支持此主机OS类型。',
  'installer.step3.hostWarningsPopup.repositories.message': '以下注册主机的操作系统类型与在“选择堆栈”步骤中选择的可用存储库不同。您可以返回“选择堆栈”步骤，选择另一个OS存储库<b>或</b>删除主机。',
  'installer.step3.hostWarningsPopup.disk':'Disk Issues',
  'installer.step3.hostWarningsPopup.disk.name':'磁盘空间不足',
  'installer.step3.hostWarningsPopup.disk.context1':'({0})主机磁盘不足.',
  ...
  'installer.step3.hostWarningsPopup.thp.message':'以下主机已启用Transparent Huge Pages（THP）。应禁用THP以避免潜在的Hadoop性能问题。',
  'installer.step3.hostWarningsPopup.firewall':'Firewall Issues',
  'installer.step3.hostWarningsPopup.firewall.message':'请将防火墙配置为允许在<a target="_blank" href="http://ambari.apache.org/current/installing-hadoop-using-ambari/content/">Ambari文档</a>的<i>配置端口</i>部分中记录的端口上进行通信',
  ...
  'installer.step3.hostWarningsPopup.alternatives.message':'应删除以下备选方案',
  'installer.step3.hostWarningsPopup.alternatives.noIssues':'不存在替代问题，应删除替代方案',
  ...
  'installer.step3.hosts.noHosts':'没有要展示的主机',
  'installer.step3.warnings.popup.header':'主机检查',
  'installer.step3.warnings.description':'对上述主机执行检查时遇到一些告警。',
  'installer.step3.warnings.linkText':'点击此处查看警告。',
  'installer.step3.noWarnings.linkText':'点击此处查看检查结果',
  'installer.step3.warnings.noWarnings':'已在{0}注册主机上传递了所有主机检查。',
  'installer.step3.warnings.fails':'对上面的{0}注册主机执行检查时遇到一些告警',
  'installer.step3.warnings.someWarnings':'已在{0}注册主机上传递了所有主机检查。注意：在注册失败的{1}主机上跳过了主机检查。',
  'installer.step3.warnings.allFailed':'在未能注册的{0}主机上跳过了主机检查。',
  ...
  'installer.step3.warning.loading':'正在检查主机的潜在问题，请稍候。。。',
  'installer.step3.registeredHostsPopup': '这些主机已在服务器中注册，但未显示在要添加的主机列表中。',
  'installer.step3.removeSelected':'移除选定项',
  ...
  'installer.step3.hostLogPopup.highlight':'单击来突显',
  ...
  'installer.step4.rangerRequirements.popup.body.requirements': '<ol><li>您必须运行一个<strong>MySQL/Oracle/Postgres/MSSQL/SQL Anywhere Server<strong>数据库实例，才能供Ranger使用</li>。<li>在此向导的“分配主机”步骤中，系统将提示您指定Ranger Admin的主机。在该主机上，<strong>必须安装DB Client</strong>，以便Ranger访问数据库。（注：这仅适用于Ranger 0.4.0）</li><li>确保在数据库服务器中从任何主机启用了对数据库管理员用户的访问。</li><li>在Ambari服务器主机上执行以下命令。根据相应JDBC驱动程序的位置，将<code>database-type</code>替换为<strong>mysql|oracle|postgres|mssql|sqlanywhere</strong>和<code>/jdbc/driver/path</code>：<pre>ambari-server setup --jdbc-db={database-type} --jdbc-driver={/jdbc/driver/path}</pre></li></ol>',
  ...
  'admin.serviceAutoStart.general.switcher': "自动重启配置",
  ...
  'admin.stackVersions.filter.upgrading': "正在升级/回滚 ({0})",
  ...
  'admin.stackUpgrade.title': "服务与版本",
  ...
  'hosts.host.deleteComponent.popup.deleteJournalNodeMsg': '你想要打开关于 <i>管理 Journal Node 向导嘛</i>. 你确定吗?',
  'hosts.host.deleteComponent.popup.warning':'<b>警告!</b> 删除集群中最后的 <i>{0}</i> 组件?</br>删除集群中的最后一个组件可能导致服务数据的永久丢失。',
  ...
  'hosts.host.delete.componentsRequireReconfigure':'使用{0}删除主机可以重新配置这些属性:',
  'host.host.addComponent.popup.dependedComponents.body': '{0}要求在同一主机上安装{1}。请先添加它们，然后再尝试添加{0}',
  'host.host.addComponent.popup.dependedComponents.header': '组件的依赖',
  'host.host.addComponent.popup.clients.dependedComponents.body': '{0}要求在同一主机上安装{1}。请先添加',
  'hosts.host.zooKeeper.configs.save.note': '这个配置是由ambari在主机上安装/删除zookeeper组件时创建的',
  'hosts.host.addComponent.securityNote':'您正在以安全模式运行集群。在继续之前，必须在{1}上为{0}设置keytab。否则，组件将无法正常启动。',
  ...
  'hosts.host.deleteComponent.popup.deleteNimbus':'删除 <i>Storm Nimbus</i> 将会重新配置 <b>nimbus.seeds</b>, <b>topology.min.replication.count</b>, <b>topology.max.replication.wait.time.sec</b> 属性，如果他们已经被定义了.',
  'hosts.host.storm.configs.save.note': '这个配置是由ambari在主机上安装/删除storm组件时创建的',
  ...
  'hosts.host.regionserver.decommission.batch1':'移除 RegionServer - 打开 drain mode',
  'hosts.host.regionserver.decommission.batch2':'移除 RegionServer - 停止 RegionServer',
  'hosts.host.regionserver.decommission.batch3':'移除 RegionServer - 关闭 drain mode',
  'hosts.host.hbase_regionserver.recommission':'重新使用 RegionServer',
  'hosts.host.hbase_regionserver.decommission':'移除 RegionServer',
  'hosts.host.hbase_regionserver.decommission.warning':'最后一个RegionServer不能被移除',
  'hosts.host.decommissioned':'已经被移除',
  'hosts.host.decommissioning':'移除中',
  'hosts.host.addComponent.JOURNALNODE': '您即将打开<i>管理日志节点向导</i>. 你确定吗?',
  'hosts.host.deleteComponent.popup.deleteHiveMetastore':'删除 <i>Hive Metastore</i> 将会重新配置:<ul><li>hive.metastore.uris</li><li>templeton.hive.properties</li></ul>',
  'hosts.host.deleteComponent.popup.deleteWebHCatServer':'您要删除 <i>WebHCat Server</i>.你确定吗?',
  'hosts.host.configs.save.note': '这个配置是由ambari在主机上安装/删除{0}组件时创建的',
  ...
  'hosts.delete.popup.body.msg1':'通过删除此主机，Ambari将忽略来自此主机的未来通信。软件包将不会从主机上删除。主机上的组件不应重新启动。如果希望将此主机重新读入集群，请确保清理主机。',
  'hosts.delete.popup.body.msg3':'如果该主机托管了Zookeeper服务器，则应该重新启动Zookeeper服务。转到<i>Services</i>页面。',
  'hosts.delete.popup.body.msg4':'<b>警告!</b>删除集群中最后一个<i>{0}</i>组件?删除集群中的最后一个组件可能导致服务数据的永久丢失。',
  'hosts.delete.popup.body.msg5':'<b>警告!</b>探针仍然启动着，所以主机仍然存在于数据库中。',
  'hosts.delete.popup.body.msg6':'要完全删除主机，首先在其上停止ambari-agent。',
  'hosts.delete.popup.body.msg7':'<b>警告!</b> {0}应该先停止以防止数据丢失吗.',
  'hosts.delete.popup.body.msg.unknownComponents':'这个主机似乎没有联机，并且Ambari-agent的通信已经丢失。',
  ...
  'hosts.passiveMode.popup.version.mismatch.multiple': '有些主机有来自非当前服务列表的组件。在将这些主机退出维护模式之前，建议您将它们的组件升级到{0}',
  'hosts.combo.search.placebolder': '根据主机和组件属性进行筛选或根据关键字进行搜索 ...',
  ...
  'dashboard': {
    'widgets': {
      'popupHeader': '个性化部件',
      'hintInfo': {
        'common': '编辑百分比阈值以更改当前饼图的颜色。 <br />输入两个数字在 0 到 {0}',
        'hint1': '编辑阈值的百分比以更改当前小部件的颜色。 <br />假设所有组件最大为 100, 并且所有的最小是 0. <br /> 因此输入的两个数字在 0 到 {0}',
        'hint2': '编辑阈值以更改当前小部件的颜色。<br /><br />请输入两个大于 0的数字。',
        'hint3': '编辑阈值以更改当前小部件的颜色。<br />这里的单位是毫秒. <br />请输入两个大于 0的数字。',
        'hint4': '编辑阈值表示数字{0}的改变会引起饼图颜色的变化. <br />如果数字 {0} 大于设定的阈值, 小部件就会显示红色警告. <br />填写数字在 0 到 {1}. '
      }
    }
  },
  ...
  'dashboard.services.hdfs.datanode.status.tooltip.live': '这是从NameNode报告的活动DataNode的数量。即使DataNode进程已启动，如果DataNode未按预期与NameNode通信，NameNode也可能会看到状态为死。这可能是由于网络问题或由于过度垃圾收集而挂起的DataNode进程等情况造成的。',
  'dashboard.services.hdfs.datanode.status.tooltip.dead': '这是从NameNode报告的死亡DataNode的数量。即使DataNode进程已启动，如果DataNode未按预期与NameNode通信，NameNode也可能会看到状态为死。这可能是由于网络问题或由于过度垃圾收集而挂起的DataNode进程等情况造成的。',
  'dashboard.services.hdfs.datanode.status.tooltip.decommission': '这是NameNode报告的当前正在停用的DataNode数。如果群集中没有足够的其他DataNode来基于dfs创建配置数量的块副本。复制属性（通常为3），DataNode可能会处于停用状态，直到更多的DataNode可用于NameNode。',
  ...
  'dashboard.services.yarn.nodeManagers.status.tooltip.lost': '这是根据ResourceManager报告丢失的NodeManager的数量。即使NodeManager进程启动，如果NodeManager未按预期与ResourceManager通信，ResourceManager也可能会将状态视为丢失。这可能是由于网络问题或由于过度垃圾收集而挂起的NodeManager进程等情况造成的。',
  'dashboard.services.yarn.nodeManagers.status.tooltip.unhealthy': '这是ResourceManager报告的不正常的NodeManager的数量。',
  'dashboard.services.yarn.nodeManagers.status.tooltip.rebooted': '这是ResourceManager报告的处于重新启动状态的NodeManager的数量。',
  'dashboard.services.yarn.nodeManagers.status.tooltip.decommissioned': '这是ResourceManager报告的已停用的NodeManager的数量。',
  ...
  'tableView.filters.itemsPerPage': '每页数目：',
  'tableView.filters.paginationInfo': '{0} - {1} of {2}',
  'tableView.filters.clearAllFilters': '清除筛选',
  'tableView.filters.showAll': '全部显示',
  'tableView.filters.filteredConfigVersionInfo': '{1}的{0}版本展示',
  'tableView.filters.filteredAlertInstancesInfo': '{1}的{0}实例展示',
  'tableView.filters.filteredLogsInfo': '{1}的{0}日志展示',
  ...
  'rollingrestart.dialog.msg.noRestartHosts': '没有{0}要执行滚动重新启动',
  'rollingrestart.dialog.msg.maintainance': '注意：维护模式下的{0} {1}将不会重新启动',
  'rollingrestart.dialog.msg.componentsAtATime': '个{0}在同一次',
  'rollingrestart.dialog.msg.timegap.prefix': '等待 ',
  'rollingrestart.dialog.msg.timegap.suffix': '秒在批次之间 ',
  'rollingrestart.dialog.msg.toleration.prefix': '最多容忍 ',
  'rollingrestart.dialog.msg.toleration.suffix': '次重启故障',
  'rollingrestart.dialog.err.invalid.batchsize': '无效的重新启动批大小： {0}',
  ...
  'rollingrestart.dialog.msg.serviceNotInMM':'注意：这将触发警报。若要抑制警报，请在触发滚动重新启动之前启用{0}的维护模式',
  ...
  'rollingrestart.context.configs.allOnSelectedHosts':'刷新选定主机上的所有配置',
  'rollingrestart.context.allForSelectedService':'重新启动{0}的所有组件',
  'rollingrestart.context.allWithStaleConfigsForSelectedService':'使用{0}的过时配置重新启动所有组件',
  'rollingrestart.context.ClientOnSelectedHost':'在{1}上重新启动{0}',
  'rollingrestart.context.allWithStaleConfigsOnSelectedHost':'使用{0}上的过时配置重新启动组件',
  ...
  'rolling.nothingToDo.header': '没有东西',
  'rolling.nothingToDo.body': '选定主机上的{0}已处于选定状态或维护模式。',

  'widget.type.gauge.description': '用于显示可用百分比表示的指标的视图。',
  'widget.type.number.description': '一个视图，用于显示可以用可选单位字段表示为单个数字的度量。',
  'widget.type.graph.description': '一种视图，用于显示可以在时间范围内以线图或面积图表示的度量。',
  ...
  'widget.create.wizard.step2.body.template':'使用任意数量的表达式和任意字符串定义模板。通过将表达式的名称用双花括号括起来，可以从模板中引用表达式。',
  'widget.create.wizard.step2.body.warning':'注意：有效运算符为+、-、*、/',
  ...
  'widget.create.wizard.step2.body.gauge.overflow.warning':'溢出！仪表只能显示介于0和1之间的数字。',
  ...
  'widget.clone.body': '您确定要克隆当前小部件<b>{0}</b>吗？',
  'widget.edit.body': '这是一个共享的小部件，编辑将为所有用户更改此小部件。您想改为克隆小部件吗？',
  ...
  'dashboard.widgets.wizard.step2.aggregateTooltip': '此数学函数将应用于计算所有主机组件中选定度量的单个值',
  ...
  'dashboard.widgets.wizard.onClose.popup.body': '您有未保存的更改。如果在此步骤退出向导，则不会保存您的更改。',
  ...
  'jobs.customDateFilter.error.laterThanNow':'指定时间在未来',
  ...
  'number.validate.moreThanMaximum': '值小于{0}',
  ...
  'config.group.selection.dialog.option.select.msg': '将更改属于所选组的主机的覆盖属性。',
  ...
  'config.group.description.default': '在{0}上创建了新配置组',
  ...
  'errorMessage.config.directory.heterogeneous': '目录格式错误，可以是"[{存储类型}]/{目录名称}"',
  'errorMessage.config.directory.default': '开头必须是斜线或驱动器，且不得包含空格',
  ...
  'utils.ajax.defaultErrorPopupBody.message': '对于API: {1}来说是通过{0}方法来接受',
  'utils.ajax.defaultErrorPopupBody.statusCode': '{0}状态码',

  'wizard.inProgress': '{0}在进行中 | 由{1}启动',

  'alerts.instance.fullLogPopup.header': '实例响应',
  'admin.addHawqStandby.button.enable': '添加HAWQ备用主机',
  'admin.addHawqStandby.closePopup':'添加HAWQ备用主机向导正在进行。您必须允许向导完成，才能使Ambari处于可用状态。如果您选择退出，则必须按照记录在案的手动说明完成或恢复添加HAWQ备用主机。确实要退出向导吗？',
  'admin.addHawqStandby.wizard.header': '添加HAWQ备用主机向导',
  'admin.addHawqStandby.wizard.step1.header': '开始',
  'admin.addHawqStandby.wizard.step1.body':'此向导将引导您完成添加HAWQ备用主机作为当前HAWQ主机备份的过程。添加HAWQ备用主机后，它将作为<i>热备用</i>，在主HAWQ主机不工作时，它将被激活<br/><br/><b>此过程将重新启动HAWQ服务。在计划的群集维护窗口期间执行此过程</b>',
  ...
  'admin.addHawqStandby.wizard.step3.confirm.dataDir.body': '在完成此过程之前，请确保在HAWQ Standby Master主机<b>{1}</b>上重命名目录<b>{0}</b>（如果存在）（例如，将其更改为{0}_old）。<br/><br/><b>如果HAWQ备用主机主机上存在{0}，则新的HAWQ待机主机可能会使用过时的数据启动，从而使群集处于不一致的状态</b> <br/><br/>单击“确认”表示您已重命名HAWQ备用主机<b>{1}</b>上的任何现有<b>{0}</b>目录。',
  ...
  'admin.removeHawqStandby.button.enable': '移除HAWQ备用',
  ...
  'admin.serviceAutoStart.save.popup.body': '您正在更改自动启动配置。单击<b>保存</b>以提交更改，或单击<b>放弃</b>以还原更改',
  ...
  'reset.ui.states.body': '只有当UI行为不正常时（例如登录时导航错误、UI被卡住且无法加载等），才应继续操作。是否确实要重置UI状态？',
